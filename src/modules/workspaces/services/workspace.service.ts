...
export class WorkspaceService {
  static async createWorkspace(userId: string, data: Partial<IWorkspace>) {
    await AccessControl.assertPlatformPermission(
      userId,
  ...
  static async getUserWorkspaces(userId: string) {
    ...
    if (AccessControl.isSuperAdmin(actor)) {
      return Workspace.find().populate("owner", "name email avatar");
    }
  ...
  static async getWorkspaceById(workspaceId: string, userId: string) {
    await AccessControl.assertWorkspacePermission(
      userId,
      workspaceId,
  ...
  static async inviteMember(
    ...
    const access = await AccessControl.assertWorkspacePermission(
      userId,
      workspaceId,
  ...
  static async getInvitation(token: string) {
    const invitation = await Invitation.findOne({ token })
      .populate("workspace", "name slug logo")
  ...
  static async updateMemberRole(
    ...
    if (!targetMember)
      throw new AppError("Workspace member not found", StatusCodes.NOT_FOUND);
    if (!AccessControl.isSuperAdmin(access.actor)) {
